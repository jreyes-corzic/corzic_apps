...
def get_sufs_brand_performance(db, uid, password, models):
	sufs_ids = od.get_ids(
		db,
		uid,
		password,
		...
		[[
			"&",
			("product_id.product_tmpl_id.sales_channel_ids", "in", [3]),
			"&",
			("product_brand_id", "!=", False),
			"&",
			("state", "in", ["done", "confirmed", "assigned", "partially_available"]),
			("x_studio_sufs_order_date", ">=", START_DATE),
		]],
	)
	sufs_list = od.read_ids(
		db,
		uid,
		password,
		models,
		"stock.move",
		sufs_ids,
		["product_brand_id", "product_uom_qty"],
	)
...
def get_sufs_brand_performance_pie_chart(db, uid, password, models):
	return get_sufs_brand_performance(db, uid, password, models)
